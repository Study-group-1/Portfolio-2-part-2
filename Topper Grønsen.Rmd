...
m6 <- update(m6, .~. + I(VISIT^2))

#- calculate performance of the model on the training data: root mean square error is a good measure. (Tip: google the function rmse())


pre <- predict(m6 , newdata = df, allow.new.levels = T)
...
#- test the performance of the models on the test data (Tips: google the functions "predict()")
pre2 <- predict(m6, newdata = clean, )

rmse(clean$CHI_MLU, pre2)
#- optional: predictions are never certain, can you identify the uncertainty of the predictions? (e.g. google predictinterval())
prei <- predictInterval(m6, newdata = clean)
...
cross_validate <- function(x, y) {
  ...
  rm <- data.frame("model" = y, "train" = trainrmse, "test" = testrmse, "diff" = testrmse - trainrmse, "meantrain" = rep(mean(trainrmse), 6), "meantest" = rep(mean(testrmse), 6), "meandiff" = rep(mean(testrmse) - mean(trainrmse), 6), "sdtrain" = rep(sd(trainrmse), 6), "sdtest" = rep(sd(testrmse), 6))
  return(rm)
}

m1_rmse <- cross_validate(m1, "m1")
...
m2_rmse <- cross_validate(m2, "m2")

m3 <- CHI_MLU ~ VISIT * Diagnosis + I(VISIT^2) + (1|SUBJ)

m3_rmse <- cross_validate(m3, "m3")

m4 <- CHI_MLU ~ VISIT + Diagnosis + types_CHI + MOT_MLU + (1|SUBJ)

m4_rmse <- cross_validate(m4, "m4")

m5 <- CHI_MLU ~ VISIT + Diagnosis * types_CHI + MOT_MLU + (1|SUBJ)

m5_rmse <- cross_validate(m5, "m5")

m6 <- CHI_MLU ~ VISIT * Diagnosis + types_CHI + MOT_MLU + (1|SUBJ)

m6_rmse <- cross_validate(m6, "m6")

return(rmse_values <- rbind(m1_rmse, m2_rmse, m3_rmse, m4_rmse, m5_rmse, m6_rmse))
...
visit_average <- function(df, diagnosis, subject){
  means_per_visit <-  rep(NA, 6)
  visits = unique(df$VISIT)
  i <-  1
  for (i in visits) {
    hej <- df %>% filter(VISIT == i & Diagnosis == diagnosis)
    means_per_visit[i] <- mean(hej$CHI_MLU)
    i <-  i + 1
  }
  td_means <- data.frame("visit" = unique(df$VISIT), "mean" = means_per_visit, "subject" = subject)
  return(td_means)
}

df_tds <- visit_average(df, "TD", "TD")

df_berns <- visit_average(bernie, "ASD", "Bern")
hejsaaaa <- rbind(df_tds, df_berns)
