...
cross_validate <- function(x, y, z) {
  ...
  rm <- data.frame("model" = y, "train" = trainrmse, "test" = testrmse, "diff" = testrmse - trainrmse, "meantrain" = rep(mean(trainrmse), k), "meantest" = rep(mean(testrmse), k), "meandiff" = rep(mean(testrmse) - mean(trainrmse), k), "sdtrain" = rep(sd(trainrmse), k), "sdtest" = rep(sd(testrmse), k))
  return(rm)
}

m1_rmse <- cross_validate(m1, "m1", 6)
...
m2_rmse <- cross_validate(m2, "m2", 6)

m3 <- CHI_MLU ~ VISIT * Diagnosis + I(VISIT^2) + (1|SUBJ)

m3_rmse <- cross_validate(m3, "m3", 6)

m4 <- CHI_MLU ~ VISIT + Diagnosis + types_CHI + MOT_MLU + (1|SUBJ)

m4_rmse <- cross_validate(m4, "m4", 6)

m5 <- CHI_MLU ~ VISIT + Diagnosis * types_CHI + MOT_MLU + (1|SUBJ)

m5_rmse <- cross_validate(m5, "m5", 6)

m6 <- CHI_MLU ~ VISIT * Diagnosis + types_CHI + MOT_MLU + (1|SUBJ)

m6_rmse <- cross_validate(m6, "m6", 6)

mse_values <- rbind(m1_rmse, m2_rmse, m3_rmse, m4_rmse, m5_rmse, m6_rmse)
